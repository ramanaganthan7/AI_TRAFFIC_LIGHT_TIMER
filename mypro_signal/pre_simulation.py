...
nextGreen = (currentGreen + 1) % noOfSignals
currentYellow = 0      # Indicates whether yellow signal is on or off
signalTimer = 20    # Green light duration (30 seconds)
yellowTimer = 5  
...
def repeat():
    global currentGreen, nextGreen, currentYellow, signalTimer, yellowTimer
    ...
    while True:
        ...
        if signalTimer > 0:
            ...
            if currentYellow == 0:
                # Switch to yellow light after green light ends
                currentYellow = 1
                signalTimer = yellowTimer
            else:
                # Switch to red light and move to the next signal
                currentYellow = 0
                signals[currentGreen].green = 0  # Set current green signal to red
                currentGreen = nextGreen  # Move to the next signal
                nextGreen = (currentGreen + 1) % noOfSignals
                signals[currentGreen].green = 20 # Set new signal to green for 30 seconds
                signalTimer = 20# Reset green light duration
                # Print the updated signal states
                print(f"Signal {currentGreen} is now green for 30 seconds")
                print(f"Next signal {nextGreen} will be green after {signalTimer} seconds")   
